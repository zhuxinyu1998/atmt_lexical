            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    for i in range(src_time_steps):
                        ...
                        lexical_feed = torch.mul(src_embeddings_sub, attn_weights_sub)
                    ...
                    print('lexical_transed',lexical_transed,lexical_transed.size())              
                    # TODO: --------------------------------------------------------------------- /CUT

            input_feed = F.dropout(input_feed, p=self.dropout_out, training=self.training)
            rnn_outputs.append(input_feed)
            
            lexical_tensor = torch.cat((lexical_tensor.clone(), lexical_transed), 0)
